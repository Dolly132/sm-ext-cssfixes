...
bool CSSFixes::SDK_OnLoad(char *error, size_t maxlength, bool late)
{
	...
	gs_Patches = {
		...
		// 10: fix server lagging resulting from too many ConMsgs due to packet spam
		{
            ...
            "\xA1\x5C\x2A\x2A\x2A\x85\xC0\x79\x2A\xA1\x2A\x2A\x2A\x2A\x85\xC0\x7F\x2A\xC3\x2A\x2A\x2A\x2A\x2A\x6B\xC0\x34\x03\x05\x2A\x2A\x2A\x2A\x8B\x50\x2A\x85\xD2\x7E\x2A\x55\xB9\x01\x00\x00\x00\xBA\x2A\x2A\x2A\x2A\x89\xE5\x83\xEC\x0C\x8D\x45\x0C\x50\x31\xC0"
		},   
		// 11: fix server lagging resulting from too many ConMsgs due to packet spam
		{
			...
			0x800, 100,
			true, "bin/libtier0_srv.so",
			"\x55\x31\xC9\xBA\x2A\x2A\x2A\x2A\x89\xE5\x83\xEC\x0C\x8D\x45\x0C\x50\x31\xC0\xFF\x75\x08"
		},
...
#else
			void* pFunctionBinary = dlopen(pPatch->pFunctionLibrary, RTLD_NOW | RTLD_NOLOAD | RTLD_GLOBAL);
#endif
			if(!pFunctionBinary)
			{
